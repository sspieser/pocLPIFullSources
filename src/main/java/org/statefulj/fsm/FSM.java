...
public class FSM<T> {
	 ...
	 */
	public State<T> onEvent(T stateful, String event, Object ... args) throws TooBusyException, UnexpectedEventException {
		...
		while(this.retryAttempts == -1 || attempts < this.retryAttempts) {
			try {
				...
				if (transition != null) {
					...
					// SSS
					// a voir: avant ou apres le test isBlocking()??
					throw new UnexpectedEventException(event);
				}
